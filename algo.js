...
var removeDuplicates = function(nums) {
    ...
    let j = 1; // Start `j` at 1 since the first element is always unique
    ...
    for (let i = 1; i < nums.length; i++) {
        if (nums[i] !== nums[i - 1]) { // If the current element is different from the previous one
            ...
            j++;                       // Increment `j` to track the next position
        }
    }

    return j; // `j` represents the number of unique elements
};
